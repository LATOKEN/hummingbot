...
class LatokenExchangeTests(TestCase):
    ...
    def test_check_network_successful(self, mock_api):
        url = latoken_utils.private_rest_url(CONSTANTS.PING_PATH_URL)
    ...
    #     url = latoken_utils.private_rest_url(CONSTANTS.ORDER_PATH_URL)
    #     regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
    #
    #     creation_response = {
    ...
    #     self.assertEqual(LatokenExchange.latoken(OrderType.LIMIT), request_data["type"])
    ...
    def test_update_time_synchronizer_successfully(self, mock_api, seconds_counter_mock):
        ...
        url = latoken_utils.private_rest_url(CONSTANTS.SERVER_TIME_PATH_URL)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
    ...
    def test_update_balances(self, mock_api):
        url = latoken_utils.private_rest_url(CONSTANTS.ACCOUNTS_PATH_URL)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        ...
        response = [
            {"id": "1e200836-a037-4475-825e-f202dd0b0e94",
             "status": "ACCOUNT_STATUS_ACTIVE",
             "type": "ACCOUNT_TYPE_WALLET",
             "timestamp": 1566408522980,
             "currency": "BTC",
    ...
    #     order = self.exchange.in_flight_orders["OID1"]
    #
    #     url = latoken_utils.private_rest_url(CONSTANTS.MY_TRADES_PATH_URL)
    #     regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
    #
    #     trade_fill = {
    #         "symbol": self.exchange_trading_pair,
    ...
    #     url = latoken_utils.private_rest_url(CONSTANTS.EXCHANGE_INFO_PATH_URL)
    #     regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
    #
    #     order_status = {
    #         "timezone": "UTC",

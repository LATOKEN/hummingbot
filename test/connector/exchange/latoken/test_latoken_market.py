...
class LatokenExchangeUnitTest(unittest.TestCase):
    ...
    def test_place_random_orders_and_cancel_all(self):
        ...
        for i in range(order_count):
            ...
            diffPrice = self.market.trading_rules[trading_pair].min_price_increment * random.randint(0, 9)

            quantize_bid_price = self.market.quantize_order_price(trading_pair, quantize_bid_price - diffPrice)
            quantize_ask_price = self.market.quantize_order_price(trading_pair, quantize_ask_price + diffPrice)

...
from hummingbot.connector.exchange.latoken.latoken_api_order_book_data_source import (
    LatokenAPIOrderBookDataSource,
)
...
class LatokenAPIOrderBookDataSourceUnitTest(unittest.TestCase):
    ...
    def test_size_snapshot(self):
        async def run_session_for_fetch_snaphot():
            ...
            assert len(result["bid"]) == self.data_source.SNAPSHOT_LIMIT_SIZE
            assert len(result["ask"]) == self.data_source.SNAPSHOT_LIMIT_SIZE
